...
; opt -polyhedral-access-function-info -analyze < %s | FileCheck %s --check-prefix=ACCESS
...
; CHECK: Domain of bb: { [] -> [(1)] }
; CHECK: Domain of bb5: { [] -> [(1)] }
; CHECK: Domain of bb11: [tmp9] -> { [] -> [(1)] : tmp9 > 0 }
; CHECK: Domain of bb12: [tmp9] -> { [i0] -> [(1)] : 0 <= i0 < tmp9 }
; CHECK: Domain of bb16: [tmp9, nj_loaded2] -> { [i0] -> [(1)] : nj_loaded2 > 0 and 0 <= i0 < tmp9 }
; CHECK: Domain of bb17: [tmp9, nj_loaded2] -> { [i0, i1] -> [(1)] : 0 <= i0 < tmp9 and 0 <= i1 < nj_loaded2 }
; CHECK: Domain of bb31: [tmp9, nj_loaded2] -> { [i0] -> [(1)] : nj_loaded2 > 0 and 0 <= i0 < tmp9 }
; CHECK: Domain of bb32: [tmp9] -> { [i0] -> [(1)] : 0 <= i0 < tmp9 }
; CHECK: Domain of bb35: [tmp9] -> { [] -> [(1)] : tmp9 > 0 }
; CHECK: Domain of bb36: { [] -> [(1)] }

; [tmp9, nj_loaded2, tmp20, tmp19] -> { Stmt_bb17[i0, i1] -> MemRef_a[-1 + tmp20 + i0, nj_loaded2 + tmp19 + i1] : i1 < -tmp19; Stmt_bb17[i0, i1] -> MemRef_a[tmp20 + i0, tmp19 + i1] : i1 >= -tmp19 };
